...
class MailModalJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        ...
        try{
            ...
            Log::info('Enviando correo a: '.$this->data->correo);

            Mail::to($this->data->correo)->send(
                new MailService(
                    $message[$this->id_service - 1][$this->number_message - 1],
                    $title[$this->id_service - 1][$this->number_message - 1],
                    $images[$this->id_service - 1][$this->number_message - 1],
                    $subject[$this->id_service - 1][$this->number_message - 1]
                )
            );

            if (isset($this->data->first_email)) {
                $this->data->first_email->update([
                    'estado' => 1,
                    'error' => 'Se envio correctamente'
                ]);
            }
        }catch (\Exception $e){
            Log::error('Error al enviar correo a: '.$this->data->correo);
            if (isset($this->data->first_email)) {
                $this->data->first_email->update([
                    'estado' => 1,
